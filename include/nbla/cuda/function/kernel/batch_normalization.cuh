...
template <typename T>
__global__ void forward_batch_mean_sqmean_kernel(
...
template <typename T>
__global__ void forward_batch_running_mean_var_kernel(
    const int size1, const int size02, const int n_procs,
    const float decay_rate, T *m, T *v, T *rm, T *rv) {
  NBLA_CUDA_KERNEL_LOOP(i1, size1) {
    m[i1] /= n_procs;
    v[i1] = v[i1] / n_procs - m[i1] * m[i1];
    rm[i1] = decay_rate * rm[i1] + (1. - decay_rate) * m[i1];
    rv[i1] =
        decay_rate * rv[i1] + (1. - decay_rate) * v[i1] * (n_procs * size02) / ((n_procs * size02) - 1);
  }
}
template <typename T>
__global__ void backward_batch_data_pre_sync_kernel(
    ...
    const T *v, const T *x, const T *g, const T *dm, const T *dv,
    T *sum_dy, T *sum_xdy) {
  NBLA_CUDA_KERNEL_LOOP(i1, size1) {
    sum_dy[i1] = 0;
    sum_xdy[i1] = 0;
...
__global__ void backward_batch_data_mean_variance_post_sync_kernel(
    const int size1, const int size02,
    const float eps,
    const T *m, const T *v, const T *g,
    const T *dm, const T *dv, const T *sum_dy, const T *sum_xdy,
    T *dmean, T *dvar) {
  NBLA_CUDA_KERNEL_LOOP(i1, size1) {
    dvar[i1] = g[i1] * sum_xdy[i1] - g[i1] * m[i1] * sum_dy[i1];
    dvar[i1] = dvar[i1] * -0.5 * std::pow(static_cast<T>(v[i1] + eps), static_cast<T>(-1.5)) + (dv ? dv[i1] : (T)0);
    dmean[i1] = g[i1] * sum_dy[i1];
    dmean[i1] = dmean[i1] * (-1 / std::sqrt(v[i1] + eps)) + (dm ? dm[i1] : (T)0);
  }
}
template <typename T>
__global__ void backward_batch_data_dx_post_sync_kernel(
    const int size102, const int size0, const int size1, const int size2,
    const int size02, const int size12, const int n, const float decay_rate, const float eps,
...
template <typename T>
__global__ void
backward_batch_gamma_beta_post_sync_kernel(const int size1_, const int size2_,
                                 const int size02_, const int size12_,
                                 const float eps_, const T *dy, const T *m,
                                 const T *v, const T *x,
                                 const T *sum_dy, const T *sum_xdy,
                                 T *db, T *dg) {
...
__global__ void backward_batch_data_mean_variance_kernel(
  ...
  NBLA_CUDA_KERNEL_LOOP(i1, size1) {
    ...
    T tmp_v = v[i1];
    dvar[i1] =
        tmp_dvar * -0.5 * pow<float>(static_cast<float>(tmp_v + eps), static_cast<float>(-1.5)) + (dv ? dv[i1] : (T)0);
...
__global__ void backward_batch_data_kernel_mean_variance_postprocess(
  ...
  if (threadIdx.x == 0) {
    const float tmp_dvar =
        mean_variance.y * (float)-0.5 * std::pow<float>(v[0] + eps, (float)-1.5) +
        (dv ? dv[axis_idx] : (T)0);
    dvar[0] = tmp_dvar;
