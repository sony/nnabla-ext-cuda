...
namespace nbla {
  ...
  protected:
    ...
    bool alloc_impl() override {
      // call bind_impl()?
      NBLA_ERROR(error_code::memory, "CudaVirtualMemory doesn't have alloc_impl().");
    }

    shared_ptr<Memory> divide_impl(size_t second_start) override {
      NBLA_ERROR(error_code::memory, "CudaVirtualMemory doesn't have divide_impl().");
    }
