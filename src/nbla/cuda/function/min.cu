...
template <typename T>
void MinCuda<T>::forward_impl_reduce(const T *x, T *y, int outer_size,
                                     int reduction_size) {
  ...
  MinPreOp<T> pre_op(x, fbuff.second, ibuff.second);
  MinPostOp<T> post_op(fbuff.second, ibuff.second, y, ind);
...
template <typename T>
void MinCuda<T>::backward_impl_reduce(const T *dy, T *dx, int outer_size,
