...
__global__ void draw_samples(const size_t size, const size_t w_size,
  ...
  NBLA_CUDA_KERNEL_LOOP(i, size) {
    const auto b = i / w_size; // batch index
...
template <typename T>
__global__ void add_gradient(size_t size, const int *idx, const T *dy, T *dx) {
  NBLA_CUDA_KERNEL_LOOP(i, size) { atomic_add(&dx[idx[i]], dy[i]); }
}
