...
namespace nbla {
  ...
  CUmemAccessDesc get_mem_access_desc(int device_id) {
    ...
    if (desc_map.find(device_id) != desc_map.end()) return desc_map[device_id];
  ...
  size_t get_allocation_granularity(int device_id) {
    ...
    if (granularity_map.find(device_id) != granularity_map.end()) return granularity_map[device_id];
    ...
    size_t chunk_size = 0;
    NBLA_CUDA_DRIVER_CHECK(cuMemGetAllocationGranularity(&chunk_size, &prop,
  ...
  CudaPhysicalMemory::~CudaPhysicalMemory() {
    if (allocated_) NBLA_CUDA_DRIVER_CHECK(cuMemRelease(handle_));
  }
  ...
  CudaVirtualMemory::CudaVirtualMemory(size_t bytes, const string &device_id,
    ...
    NBLA_CHECK(bytes == round_up_by_chunk(bytes, std::stoi(device_id)),
               error_code::memory, "Bytes size passed is not a multiple of chunk size.");
    dev_ptr_ = 0ULL;
...
//    print("[CudaVirtualMemory] Dtor()", byte_to_human_readable(this->bytes()));
  ...
  void CudaVirtualMemory::free_virtual_address() {
    if (dev_ptr_) {
      ...
      NBLA_CUDA_DRIVER_CHECK(cuMemAddressFree(dev_ptr_, this->bytes()));
    }
  ...
  void CudaVirtualMemory::bind_impl() {
    ...
    NBLA_CUDA_DRIVER_CHECK(cuMemAddressReserve(&dev_ptr_, this->bytes(), 0ULL, 0ULL, 0ULL));
    ...
    for (auto &m: p_memories_) {
      ...
      NBLA_CUDA_DRIVER_CHECK(cuMemMap(dev_ptr_ + mapped_bytes, pm->bytes(),
                                      0ULL, pm->get_handle(), 0ULL));

      mapped_bytes += pm->bytes();
    }

    auto accessDesc = get_mem_access_desc(d_id);
    NBLA_CUDA_DRIVER_CHECK(cuMemSetAccess(dev_ptr_, this->bytes(),
                                          &accessDesc, 1ULL));
  ...
  DeviceMemoryState CudaVirtualMemory::get_device_memory_state() {
    ...
    if (status == cudaSuccess) return DeviceMemoryState::Unlocked;
    else if (status == cudaErrorNotReady) return DeviceMemoryState::Locked;
    else NBLA_CUDA_CHECK(status); // raise by message
  }
