...
void cublas_getrf_batched<double>(
    cublasHandle_t handle, int n, double **x, int lda, int *pivot,
    int *info, int batchSize) {
  NBLA_CUBLAS_CHECK(cublasDgetrfBatched(handle, n, x, lda,
                                        pivot, info, batchSize));
}
template <>
void cublas_getrf_batched<float>(
    cublasHandle_t handle, int n, float **x, int lda, int *pivot,
    int *info, int batchSize) {
  NBLA_CUBLAS_CHECK(cublasSgetrfBatched(handle, n, x, lda,
                                        pivot, info, batchSize));
}
