...
template <typename T>
void GRUCudaCudnn<T>::copy_weight_bias_to_params(Tcu *params, const Tcu *w_init,
                                                 const Tcu *weight,
                                                 const Tcu *bias,
                                                 bool weight_exists,
                                                 bool bias_exists) {
  for (int64_t layer_id = 0; layer_id < this->num_layers_ * num_directions_;
    ...
    for (int64_t lin_layer_id = 0; lin_layer_id < num_lin_layers_;
      ...
      if (layer_id / num_directions_ == 0) {
        if (lin_layer_id < 3) {
          inweight_offset =
              layer_id * (input_dim_ + hidden_size_) * 3 * hidden_size_ +
          ...
          inweight_offset =
              layer_id * (input_dim_ + hidden_size_) * 3 * hidden_size_ +
              (lin_layer_id - 3) * hidden_size_ * (input_dim_ + hidden_size_) +
              input_dim_;
          NBLA_CUDA_LAUNCH_KERNEL_SIMPLE(
              (kernel_forward_copy_weights<Tcu>),
              weight_offsets_[param_index].second, w_init + inweight_offset,
          ...
          if (this->num_layers_ > 1 && weight_exists) {
            inweight_offset =
                (layer_id - num_directions_) *
                    (num_directions_ * hidden_size_ + hidden_size_) * 3 *
                    hidden_size_ +
...
template <typename T>
void GRUCudaCudnn<T>::copy_params_to_gradients(
...
template <typename T>
void GRUCudaCudnn<T>::setup_impl(const Variables &inputs,
...
template <typename T>
void GRUCudaCudnn<T>::forward_impl(const Variables &inputs,
                                   const Variables &outputs) {
...
template <typename T>
void GRUCudaCudnn<T>::forward_impl_training(const Variables &inputs,
                                            const Variables &outputs) {
...
template <typename T>
void GRUCudaCudnn<T>::forward_impl_inference(const Variables &inputs,
                                             const Variables &outputs) {
  cuda_set_device(this->device_);
  auto cudnn_handle =
      SingletonManager::get<CudnnHandleManager>()->handle(this->device_);
...
template <typename T>
void GRUCudaCudnn<T>::backward_impl(const Variables &inputs,
                                    const Variables &outputs,
